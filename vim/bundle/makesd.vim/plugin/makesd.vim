...
" Added by B.Leopold 2010-11-17
function _make_frame_comment(start_line, end_line, line_width, linechar, str)
	let middle_sz=a:line_width - len(a:start_line . a:end_line)
	let mkr=a:start_line . repeat(a:linechar, middle_sz) . a:end_line
	let slen=len(a:str)
	let title_left_sz=((middle_sz / 2) - (slen / 2))
	let title_line=a:start_line . repeat(' ', title_left_sz) . a:str
	let title_right_sz=a:line_width - len(title_line) - len(a:end_line)
	let title_line=title_line . repeat(' ',title_right_sz) . a:end_line
...
function _cmd_make_frame_comment(str)
	call _make_frame_comment('#', '#', 80, '-', a:str )
endfunction

command! -nargs=+ CommentFrame :call _cmd_make_frame_comment(<args>)
" END - Added by B.Leopold 2010-11-17
